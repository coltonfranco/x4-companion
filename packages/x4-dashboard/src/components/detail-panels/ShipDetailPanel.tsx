...
import { PageLoaderPreset } from "../layout/PageLoader";
...
export function ShipDetailPanel({ shipId, factions }: { shipId: string; factions: FactionSummary[] }) {
  ...
  const maxTravel = classMax?.travel_max;
  const maxBoost = classMax?.boost_max;
  const maxAccel = classMax?.accel_max;
  const maxShield = classMax?.shield_capacity_max;
  const maxRegen = classMax?.shield_recharge_max;
  const maxCargo = classMax?.cargo_volume;
  const maxDps = classMax?.dps_max;
  const maxRange = classMax?.range_max;
  const maxCrew = classMax?.crew_max;
  const maxMissile = classMax?.missile_max;
          ...
          <div className="bg-muted/10 border border-border/50 rounded-lg overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-6 p-5 pt-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <ShipDetailStatRow label="Crew Capacity" min={null} max={data.people_capacity} maxVal={maxCrew!} unit="Crew" />
              ...
              <div className="flex flex-col gap-2">
                ...
                <ShipDetailStatRow label="Missile Cap" min={null} max={data.missile_storage} maxVal={maxMissile!} unit="Ms" />
